...
def save_csv(output_path, qualifying_loans):
    """Saves the CSV file from path provided by user input.
        Validates the file is written and notifies the user

    Args:
        output_path (Path): The csv file output path from user input.
        qualifying_loans : List of all qualifying loans from calculations.

    Saves:
        A list that contains all of the rows of qualifying loans for a customer.
    """
